...
| IPA | ".ipa file" | Apple | The installable, uploadable iOS package format. Apple's documentation calls it simply an ".ipa file". Two expansions are **not** Apple's and are not used here: "iOS App Store Package" is third-party usage, and "app archive" is Microsoft's phrase — in Apple's own vocabulary an archive is the `.xcarchive` you export an `.ipa` *from*, which is a different artefact. |

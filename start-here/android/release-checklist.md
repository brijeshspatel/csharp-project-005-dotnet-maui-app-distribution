...
| 8 | Signing | Create and protect an upload keystore. [`keytool -genkeypair` command](../../platforms/android/direct-apk-distribution/README.md#6-how-to-obtain-the-prerequisites); [why upload and app signing keys differ](../../platforms/android/google-play-public-release/README.md#7-security-model) | — | Keystore exists, backed up, never committed | Confirm it is **outside the repository**; `.gitignore` is a backstop, not the control | ☐ |
| 9 | Signing | [Sign for real release](../../platforms/android/google-play-public-release/README.md#10-sign): **`-p:AndroidKeyStore=true`** plus the `AndroidSigningKeyStore` properties. **Without `AndroidKeyStore=true` the signing properties are silently ignored** and you get a debug-signed package | 6, 8 | A signed App Bundle | Read the certificate off the `.aab` with `jarsigner -verify -certs`, and confirm it is *yours*, not a debug one. **`apksigner` cannot read an App Bundle** — Google documents it as APK-only, so use it only for the [direct APK channel](../../platforms/android/direct-apk-distribution/README.md#14-validate) | ☐ |

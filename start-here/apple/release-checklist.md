...
| 3 | Application setup | [Fix the Bundle ID](../../platforms/apple/app-store-public-release/README.md#8-application-preparation) as the Application ID, matching `Info.plist` | — | One identifier, used everywhere | Grep the `.csproj` and `Info.plist` and confirm they agree | ☐ |
...
| 6 | ⚠️ **Gate** | **Back up the certificate's private key** and record where the profile lives | 4, 5 | A recoverable copy exists off this machine | Restore it somewhere else and confirm it imports | ☐ |
| 7 | Build | `dotnet publish -f net10.0-ios -c Release` | 3 | Managed and AOT output compile | Read the exit code **and** see step 8 | ☐ |
| 8 | ⚠️ **Gate** | **Confirm no `.ipa` was produced, and understand why** | 7 | The publish folder is **empty** | `ls bin/Release/net10.0-ios/ios-arm64/publish/`. The build **prints that it created a package it did not write**. Never trust that line | ☐ |
| 9 | 🔐 Signing | [Sign for real distribution](../../platforms/apple/app-store-public-release/README.md#10-sign) with `-p:CodesignKey` and `-p:CodesignProvision` | 5, 7 | A signed archive is produced | The command exits 0 and names your identity | ☐ |
...
| 15 | 🧪 Testing | [External TestFlight](../../platforms/apple/testflight/README.md) where used: up to 10,000 testers | 14, or 13 if skipping internal | Beta review passes for the first build of a version | The build shows as approved for external testing | ☐ |
| 16 | ⚠️ **Gate** | **Confirm store readiness**: privacy manifest, App Privacy answers, age rating, listing metadata | 13 | Every item complete | Walk the App Store Connect checklist. **App Review rejects incomplete submissions** | ☐ |

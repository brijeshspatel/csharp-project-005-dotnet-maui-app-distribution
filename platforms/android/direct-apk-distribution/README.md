...
**Google Play Protect still applies.** It scans installed apps, including sideloaded ones. It
**automatically blocks** installation of apps obtained outside a store that request certain
sensitive permissions — access to messages and notifications among them — because those permissions
...
|---|---|---|
| `com.companyname.distributionsample.apk` | 28,945,466 bytes | No — `keytool -printcert` reports `Not a signed jar file` |
...
2. **The signed APK carries the intended key.** `keytool -printcert -jarfile` reports the
   certificate created for this verification, with `SHA384withRSA`, not a debug certificate.

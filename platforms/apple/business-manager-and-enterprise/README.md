...
⚠️ **WARNING — these are not interchangeable, and Apple enforces the difference.** Apple states the
...
⚠️ **A separate Apple Account is required** if you are already enrolled in the standard Apple
...
⚠️ **An in-house build has no technical device restriction whatsoever.** Treat the `.ipa` as a
...
⚠️ **`dotnet publish -f net10.0-ios -c Release` produces no `.ipa`.** It exits 0 and prints
...
⚠️ **WARNING — this choice is close to irreversible.** Once the app is approved, the distribution

...
⚠️ **WARNING — two different device limits are published, and the stricter reading is not the
...
⚠️ **Adding a device later does not update a profile you already generated.** You must regenerate
...
⚠️ **Do not fall back to `dotnet publish -f net10.0-ios -c Release` and assume you have a package.**
